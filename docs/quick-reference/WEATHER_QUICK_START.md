...
# Dynamically find project root by looking for README.md or setup.py
def find_project_root():
    current = Path.cwd()
    for parent in [current] + list(current.parents):
        if (parent / 'README.md').exists() or (parent / 'setup.py').exists():
            return parent
...
# Load weather data
loader = DataLoader(project_root / 'Data' / 'barber')
